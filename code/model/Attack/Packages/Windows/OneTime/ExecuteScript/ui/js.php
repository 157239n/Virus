...
<script>
    ...
    class Extra {
         ...
         */
        export() {
            return {"identifier": this.identifier, "content": htmlspecialchars_decode(this.content)}
        }
    }
    ...
    let extras = new Extras([<?php echo implode(", ", map($attack->getExtras(), fn($extra) => "new Extra(\"" . $extra["identifier"] . "\", `" . base64_encode($extra["content"]) . "`)")); ?>]);
