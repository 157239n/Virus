...
class Demos {
    ...
    public function render(array $slides): void { ?>
        <div id="demo" class="w3-card" style="width: 100%; position: relative">
            ...
            if (isset($sSlides["virus-0"])) $this->text("virus-0", "<h2>Attack types</h2><ul>
                <li><b>One time</b>: These are attacks that run once, report back, and you can view the results. Pretty straightforward.</li>
                <li><b>Session</b>: These are attacks that run for a short period of time (~5 to 30 minutes), giving you results in realtime, and once that period of time is over, you can view every past results. These can be things like, monitor their screen every 5 seconds and streaming back.</li>
                <li><b>Background</b>: These are attacks that can run indefinitely if you choose to. Old data past 2 days will be deleted, new data will keep streaming in and you can decide what data to keep. These can be things like monitoring when they plugged in a USB, monitoring their screen 24/7, or seeing what programs they are running.</li></ul>");
